    ...
    <div className="section-3 mt-20 h-auto bg-[#fffcf2]">
      <div className="py-24 lg:py-32 lg:grid md:px-10 md:grid-cols-[.7fr,1.3fr] lg:grid-cols-[1.3fr,1fr]">
        ...
        <div className="container__text-box text-center lg:text-left">
          <h3 style={{textShadow: '1px -1px #fffcf2'}} className="text-[1.6rem] md:max-w-[30ch] mx-auto
          ...
          <div className="flex justify-center lg:justify-start lg:mx-0 gap-4 items-center mx-4">
            <button className="bg-[#3f3d56] p-2 max-w-[200px] rounded-md flex gap-2 items-center w-full justify-center" onClick={() => setOpenModal(true)}>
              <Icon className="text-4xl lg:text-4xl" icon="logos:google-play-icon" />
              <div className="text-white grid text-left w-fit">
                <span className="text-lg">Get it on</span>
                <span className="text-lg block w-fit font-bold !text-nowrap">Google Play</span>
              </div>
            </button>
            <button className="!bg-[#3f3d56] max-w-[200px] p-2 h-fit rounded-md w-full flex gap-2 items-center justify-center" onClick={() => setOpenModal(true)}>
                <Icon className="text-4xl lg:text-4xl text-white" icon="file-icons:apple"/>
                <div className="text-white grid text-left w-fit">
                  <span className="text-lg">Get it on</span>
                  <span className="text-lg block w-fit font-bold !text-nowrap">Apple Store</span>
                </div>
            </button>
            <CookingModal open={openModal} onClose={() => setOpenModal(false)}/>
          </div>
        </div>
      </div>
    </div>
